...
@export_range(3, 128) var sides := 32 : set = set_sides
@export_range(0.0, 360.0) var angle_degrees := 360.0 : set = set_angle_degrees

@export var stroke_color := Color(0.4, 0.5, 1.0) : set = set_stroke_color
@export_range(0.0, 1000.0) var stroke_width := 10.0 : set = set_stroke_width
@export_enum("Sharp", "Bevel", "Round") var stroke_joint_mode: int = Line2D.LINE_JOINT_SHARP : set = set_stroke_joint_mode
@export_range(0.0, 1000.0) var stroke_sharp_limit := 2.0 : set = set_stroke_sharp_limit
@export_range(1, 32) var stroke_round_precision := 8 : set = set_stroke_round_precision
